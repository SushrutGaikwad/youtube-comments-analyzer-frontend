  ...
  chrome.tabs.query({ active: true, currentWindow: true }, async (tabs) => {
    ...
    if (match && match[1]) {
      ...
      if (predictions) {
        ...
        const labelMapping = { "2": -1, "0": 0, "1": 1 };
        // Correct label mapping: 2 → -1 (Negative), 0 → 0 (Neutral), 1 → +1 (Positive)
        const totalSentimentScore = predictions.reduce(
          (sum, item) => sum + labelMapping[item.sentiment],
          0
        );
        // Compute total sentiment score using the mapped values
